...
#[test]
fn exclude_newer_before_supported_r_errors_before_rig_selection() {
    let script = unique_path("ir-exclude-newer-before-r4", "R");
    fs::write(
        &script,
        concat!(
            "#| exclude-newer: 2020-03-01\n",
            "\n",
            "cat('ignored\\n')\n",
        ),
    )
    ...
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(stderr.contains("before R 4.0.0"), "{}", output_text(&out));
    assert!(stderr.contains("2020-04-24"), "{}", output_text(&out));
