...
Supported version specs include `pkg`, `pkg>=1.0`, and `pkg==1.2`. Source refs are resolved with pak and then passed through to renv, including GitHub, GitLab, Bitbucket, git, URL, and local package refs in forms both tools accept. GitHub `/tree/` URLs are normalized to `github::owner/repo[/subdir]@ref` when that mapping is unambiguous. Local packages may be written as `local::path`, as a bare file-system path, or with a package prefix such as `mypkg=./path`. `packages` must be a YAML sequence; each sequence entry is one package ref. Omit `packages`, use `packages: []`, or use `packages: null` when no package refs are needed. Unknown keys are ignored after YAML parsing, so future metadata such as `sys-reqs` can be present before `ir` supports it.
